            ...
            if ($row['username'] != "Name") {
                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if (!$row = mysqli_fetch_assoc($resultl)) {
                        ...
                        $sql = "INSERT INTO users_logs (username, serialnumber, fingerprint_id, checkindate, timein, timeout, parent_number, student_number, email, student_class, status) VALUES (? ,?, ?, CURDATE(), CURTIME(), ? ,?, ?, ?, ?, ?)";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            $timeout = "0";
                            mysqli_stmt_bind_param($result, "sdis", $Uname, $Number, $fingerID, $timeout,  $parent_number, $student_number, $email, $student_class, $status);
                            mysqli_stmt_execute($result);
