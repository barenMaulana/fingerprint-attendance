    ...
    if (!mysqli_stmt_prepare($result, $sql)) {
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($row['username'] == "Name") {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email) && !empty($parent_name) && !empty($parent_number) && !empty($student_class) && !empty($student_number)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email) && !empty($Timein)) {
                                ...
                                $sql = "UPDATE users SET username=?, serialnumber=?, gender=?, email=?, time_in=?, parent_number=?, parent_name=?, student_name=?, student_class=?, student_year=?, student_number=? WHERE fingerprint_select=1";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_select_Fingerprint";
                                    exit();
                                } else {
                                    mysqli_stmt_bind_param($result, "sdsss", $Uname, $Number, $Gender, $Email, $Timein, $parent_number, $parent_name, $student_name, $student_class, $student_year, $student_number);
                                    mysqli_stmt_execute($result);
